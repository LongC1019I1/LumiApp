<template>
  <span class="payPassword">
    ...
    <div
      @focus="focus"
      class="sixDigitPassword"
      :class="val === '' ? 'devicePinFirst' : ''"
      :tabindex="tabindex"
    >
      <i
        :key="i"
        :style="
          index === val.length ? 'border: 1px solid white !important ' : ''
        "
        v-for="(index, i) in length"
      >
        <b
          v-show="index <= val.length"
    ...
    keydown(e) {
      ...
      if (this.val.length === 4) {
        if (
          ...
          this.val = "";
          $alertify.error(this.$t("secure.pin.pin-not-valid"));
        }
      }
