<template>
  <div class="pb-5 schedule-speaker" :speakerStatus="speakerStatus">
    <v-select
      :class="{'pb-5 mb-5': status!==PLAY_CONTROL.resume}"
      :options="statusList"
      ...
      :clearable="false"
      :searchable="false"
      @input="isShowChooseSongBox = false; isShowVolume = false; updateStatus()"
    >
    ...
    <template v-if="status===PLAY_CONTROL.resume">
      <div
        class="my-3 d-flex align-center c-p"
        @click="isShowChooseSongBox=!isShowChooseSongBox;updateStatus()"
      >
        <i
          class="mdi font-20"
          :class="isShowChooseSongBox?'mdi-checkbox-marked':'mdi-checkbox-blank-outline'"
        ></i>
      ...
      <div
        v-if="isShowChooseSongBox && !isLoadingMusic &&listMusic&&listMusic.length"
        class="song-box my-3"
      >
        <template v-for="(songItem, index) in listMusic">
          <div
            ...
            @click="song=index+1; updateStatus()"
          >
            <div class="icon">
              <i class="fa fa-music"></i>
            </div>
            <div class="pl-2 w-80">
              <div class="text-overflow-hidden">{{songItem.Title}}</div>
              <div class="text-overflow-hidden">{{songItem.Artist}}</div>
            </div>
          </div>
        </template>
      </div>
      <div
        v-if="isShowChooseSongBox&& !isLoadingMusic &&(!listMusic||!listMusic.length)"
      >{{$t('dashboard.text.noSongInList')}}</div>
      <div class="my-3 c-p d-flex align-center" @click="isShowVolume=!isShowVolume;updateStatus()">
        <i
          class="mdi font-20"
          :class="isShowVolume?'mdi-checkbox-marked':'mdi-checkbox-blank-outline'"
        ></i>
        <span class="pl-2">{{$t('scene.volumeSlider')}}</span>
      </div>
      <div v-if="isShowVolume" class="volume-box mt-3 px-2">
        <vue-slider v-model="volume" @drag-end="updateStatus" :drag-on-click="true"></vue-slider>
      </div>
    </template>
  </div>
</template>
